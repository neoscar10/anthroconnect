<div x-data="{ isModalOpen: false }" @open-modal.window="isModalOpen = true; document.body.style.overflow = 'hidden'" @close-modal.window="isModalOpen = false; document.body.style.overflow = 'auto'" class="relative">
    ...
    <div class="flex justify-between items-end mb-12">
        <div>
            <h1 class="font-headline text-4xl text-on-surface mb-2 italic">Core Concepts</h1>
            <p class="font-body text-on-surface-variant text-lg">Manage the encyclopedia of definitions and foundational concepts.</p>
        </div>
        ...
        <div class="p-6 border-b border-outline-variant/10 flex flex-wrap gap-4 items-center justify-between bg-surface-container-low/30">
            <div class="flex gap-4 items-center flex-wrap">
                <div class="relative flex-1 max-w-sm">
                    ...
                    <input wire:model.live.debounce.300ms="search" class="w-full bg-white border border-outline-variant/20 rounded-xl pl-10 pr-4 py-2.5 text-xs focus:ring-2 focus:ring-primary transition-all shadow-sm" placeholder="Search concepts..." type="text"/>
                </div>
        ...
        <div class="overflow-x-auto pb-24">
            <table class="w-full text-left border-collapse">
                <thead class="bg-surface-container-low/50">
                    <tr>
                        <th class="px-6 py-4 label-md text-on-surface-variant uppercase tracking-widest text-[10px]">Concept Title</th>
                        <th class="px-6 py-4 label-md text-on-surface-variant uppercase tracking-widest text-[10px]">Short Summary</th>
                ...
                <tbody class="divide-y divide-outline-variant/10">
                    @forelse($concepts as $concept)
                        <tr class="hover:bg-surface-container-low/20 transition-colors group">
                            <td class="px-6 py-4">
                                <p class="font-headline font-bold text-on-surface mb-0.5 leading-tight">{{ $concept->title }}</p>
                                <p class="text-[10px] text-stone-400 font-mono italic">{{ $concept->slug }}</p>
                                @if($concept->is_upsc_relevant)
                                    <span class="badge bg-warning-subtle text-warning text-[8px] uppercase font-bold px-2 py-0.5 rounded mt-1 inline-block">UPSC</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-xs text-on-surface-variant max-w-sm truncate">
                                {{ Str::limit($concept->short_description, 60) }}
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 rounded-full text-[9px] font-bold uppercase {{ $concept->status == 'active' ? 'bg-primary-container text-on-primary-container' : 'bg-surface-container-highest text-on-surface-variant' }}">
                                    {{ $concept->status }}
                                </span>
                            </td>
                             <td class="px-6 py-4 text-right whitespace-nowrap overflow-visible">
                                 <div x-data="{ open: false }" class="relative inline-flex justify-end items-center">
                                     ...
                                     <div x-show="open" 
                                         ...
                                         <button type="button" @click="open = false; $dispatch('open-modal')" wire:click="openModal({{ $concept->id }})" class="w-full text-left px-4 py-3 text-[10px] uppercase tracking-widest font-bold text-on-surface-variant hover:bg-surface-container-low transition-colors flex items-center gap-2">
                                         ...
                                         <button type="button" @click="open = false; $dispatch('open-delete-modal', { 
                                                     ...
                                                     action: { type: 'livewire', component: '{{ $this->getId() }}', method: 'delete', params: [{{ $concept->id }}] } 
                                                 })" 
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-20 text-center">
                                <div class="flex flex-col items-center">
                                    <span class="material-symbols-outlined text-5xl text-outline-variant/30 mb-2">library_books</span>
                                    <p class="text-on-surface-variant font-headline text-2xl italic font-bold">No Concepts Found.</p>
                                    <p class="text-[10px] uppercase tracking-widest text-stone-400 mt-2 max-w-xs leading-relaxed">Adjust your filters or add a new core concept to begin compiling the encyclopedia.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <div x-show="isModalOpen" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" x-cloak>
        ...
        <div x-show="isModalOpen"
             wire:ignore.self
             class="bg-surface-container-lowest rounded-[32px] shadow-2xl ring-1 ring-white/10 w-full max-w-4xl overflow-hidden relative z-10 flex flex-col max-h-[90vh]"
             ...
             x-transition:leave-end="scale-95 opacity-0 translate-y-4">
                ...
                <form wire:submit.prevent="save" class="flex flex-col h-full min-h-0">
                    <div class="p-8 border-b border-outline-variant/10 flex justify-between items-center bg-surface-container-low/30 shrink-0">
                        <div>
                            <h4 class="font-headline text-2xl text-on-surface italic font-bold leading-tight">{{ $conceptId ? 'Edit Core Concept' : 'New Core Concept' }}</h4>
                        </div>
                    ...
                    <div class="flex-1 overflow-y-auto p-10 space-y-8 no-scrollbar min-h-0">
                        <div class="space-y-2">
                            <label class="text-[10px] uppercase font-bold text-on-surface-variant tracking-widest">Concept Title</label>
