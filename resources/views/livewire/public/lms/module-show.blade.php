<div class="min-h-screen bg-stone-50 pb-32">
    <!-- Module Hero -->
    <header class="relative bg-stone-900 pt-24 pb-48 overflow-hidden">
        <div class="absolute inset-0 opacity-10 ethno-pattern"></div>
        <div class="max-w-7xl mx-auto px-6 relative z-10">
            <nav class="flex items-center gap-2 text-[10px] uppercase font-bold tracking-[0.2em] text-stone-500 mb-12">
            ...
            <div class="grid lg:grid-cols-12 gap-16 items-center">
                <div class="lg:col-span-12">
                    ...
                    <h1 class="font-headline text-5xl md:text-7xl text-white italic font-bold mb-8 leading-tight">{{ $module->title }}</h1>
                    <p class="text-stone-300 text-lg md:text-xl max-w-3xl font-light leading-relaxed mb-10">
                    ...
                    <div class="flex flex-wrap gap-4">
                        ...
                        @endphp
                        @if($firstLesson)
                            <button wire:click="openLesson('{{ $firstLessonSlug }}')" class="bg-primary text-white px-10 py-5 rounded-2xl font-bold uppercase tracking-widest text-[10px] shadow-2xl shadow-primary/30 hover:-translate-y-1 transition-all flex items-center gap-3 group">
                                Start Exploration
    ...
    <div class="max-w-7xl mx-auto px-6 -mt-32 relative z-20">
        <div class="grid lg:grid-cols-12 gap-10">
            ...
            <div class="lg:col-span-8 space-y-10">
                <!-- Overview Section -->
                <section class="bg-white rounded-[32px] p-8 md:p-12 border border-stone-200 shadow-sm">
                    <h2 class="font-headline text-3xl font-bold italic text-stone-900 mb-8 border-b border-stone-100 pb-6 uppercase tracking-tight">Curriculum Briefing</h2>
                    <div class="prose prose-stone max-w-none text-stone-600 leading-relaxed italic text-lg">
                ...
                <!-- Curriculum List -->
                <section class="bg-white rounded-[32px] p-8 md:p-12 border border-stone-200 shadow-sm">
                    <h2 class="font-headline text-3xl font-bold italic text-stone-900 mb-10 uppercase tracking-tight">Academic Units</h2>
                    ...
                    <div class="space-y-4">
                        @foreach($lessons as $index => $lesson)
                            ...
                            <div wire:click="openLesson('{{ $lesson->slug }}')" 
                                class="group p-6 bg-stone-50 rounded-2xl border border-stone-100 flex items-center gap-6 cursor-pointer hover:bg-white hover:border-primary/20 hover:shadow-lg transition-all duration-300">
                                <div class="w-12 h-12 rounded-xl bg-white border border-stone-200 flex items-center justify-center shrink-0 font-headline font-bold italic text-lg text-stone-300 group-hover:text-primary transition-colors">
                                    {{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-3 mb-1">
                                        <h4 class="font-headline text-xl font-bold text-stone-900 italic truncate group-hover:text-primary transition-colors">{{ $lesson->title }}</h4>
                                ...
                                <div class="flex items-center gap-6 shrink-0">
                                    <div class="flex flex-col items-end">
                                        <span class="text-[10px] font-bold uppercase tracking-widest text-stone-400">{{ $lesson->video_source_type == 'upload' ? 'Video Lecture' : 'External Seminar' }}</span>
                                        <span class="text-[9px] text-stone-400 italic">{{ $lesson->duration_minutes ?? '0' }}m duration</span>
                                    </div>
                                    @if($isLocked)
                                        <div class="w-10 h-10 rounded-full bg-stone-200 flex items-center justify-center shadow-inner group-hover:bg-primary group-hover:text-white transition-all">
                                            <span class="material-symbols-outlined text-sm" style="font-variation-settings: 'FILL' 1;">lock</span>
                                        </div>
                                    @else
                                        <div class="w-10 h-10 rounded-full bg-white border border-stone-200 flex items-center justify-center group-hover:border-primary group-hover:bg-primary group-hover:text-white transition-all">
                    ...
                    <div class="relative z-10 text-center">
                        <div class="relative inline-flex items-center justify-center mb-6">
                            <svg class="w-24 h-24 transform -rotate-90">
                                ...
                                <circle cx="48" cy="48" r="40" stroke="currentColor" stroke-width="4" fill="transparent" stroke-dasharray="251.2" stroke-dashoffset="251.2" class="text-orange-500 transition-all duration-1000" />
                            </svg>
                            <span class="absolute text-2xl font-headline font-bold italic">0%</span>
                        </div>
                        <h4 class="font-headline text-xl font-bold italic mb-2">Academic Journey</h4>
                        <p class="text-[10px] text-stone-400 uppercase tracking-[0.2em]">0 of {{ $lessons->count() }} Lessons Completed</p>
                    </div>
                </section>
                @endif
                @endauth
                ...
                <section class="bg-white rounded-[32px] p-8 border border-stone-200 shadow-sm">
                    ...
                    <div class="space-y-4">
                        @foreach($resources as $resource)
                            ...
                            @endphp
                            <div wire:click="downloadResource({{ $resource->id }})" class="group flex items-start gap-4 p-4 rounded-2xl hover:bg-stone-50 transition-colors cursor-pointer">
