...
use App\Services\Payment\Actions\ProcessCashfreeWebhookAction;
use Exception;
...
class ProcessCashfreeWebhookJob implements ShouldQueue
{
    ...
    public function handle(ProcessCashfreeWebhookAction $action): void
    {
        ...
        $event = $this->payload['type'] ?? 'unknown';
        Log::channel('payments')->info("Processing queued Cashfree Webhook Job: Log ID [{$this->logId}], Event [{$event}]");
